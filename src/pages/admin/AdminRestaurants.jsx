...
import { Plus, Pencil, Trash2, X, Image, CheckCircle, XCircle, UtensilsCrossed } from 'lucide-react'
...
export default function AdminRestaurants() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const r = await restaurantsApi.adminList()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else { await restaurantsApi.update(modal.id, payload); toast.success('Restaurant modifié !') }
      setModal(null); load()
    } catch (err) {
  ...
  const handleValider = async (id) => {
    try { await restaurantsApi.valider(id); toast.success('Validé !'); load() }
    catch { toast.error('Erreur') }
  }
  const handleRejeter = async (id) => {
    try { await restaurantsApi.rejeter(id); toast.success('Rejeté'); load() }
    catch { toast.error('Erreur') }
  }
  const handleDelete = async (id) => {
    if (!confirm('Supprimer ce restaurant ?')) return
    try { await restaurantsApi.delete(id); toast.success('Supprimé'); load() }
  ...
  const handleGalerie = async (e) => {
    ...
    fd.append('status', '1')
    try {
      await galeriesApi.createRestaurant(fd)
    ...
    <div className="admin-page">
      ...
      {loading ? <div className="center-spinner"><Spinner /></div> : (
        <table className="admin-table">
          <thead><tr><th>Nom</th><th>Adresse</th><th>Cuisine</th><th>Région</th><th>Statut</th><th>Actions</th></tr></thead>
          <tbody>
              ...
              <tr key={restaurant.id}>
                ...
                <td>
                  <div className="admin-table__actions">
                    {restaurant.status !== 'valide' && <button className="admin-icon-btn admin-icon-btn--success" title="Valider" onClick={() => handleValider(restaurant.id)}><CheckCircle size={15} /></button>}
                    {restaurant.status !== 'rejete' && <button className="admin-icon-btn admin-icon-btn--danger" title="Rejeter" onClick={() => handleRejeter(restaurant.id)}><XCircle size={15} /></button>}
                  ...
                  {galModal.galeries.map(g => (
                    <div key={g.id} style={{ position: 'relative' }}>
                      ...
                      <button
                        ...
                        onClick={async () => { await galeriesApi.deleteRestaurant(g.id); load(); setGalModal(r => ({ ...r, galeries: r.galeries.filter(x => x.id !== g.id) })) }}
                      >✕</button>
                    </div>
                  ))}
                </div>
