...
import { Plus, Pencil, Trash2, X, Image, CheckCircle, XCircle, Route } from 'lucide-react'
import { transportsApi, galeriesApi, regionsApi, trajetsApi, villesApi } from '../../api/services'
...
const emptyForm = {
  ...
  latitude: '', longitude: '', type_transport: '', capacite: '',
...
export default function AdminTransports() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const r = await transportsApi.adminList()
  ...
  const openEdit = (transport) => {
    setForm({
      ...
      status: transport.status || 'en_attente', id_region: transport.id_region || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modal === 'create') { await transportsApi.create(payload); toast.success('Transport créé !') }
      else { await transportsApi.update(modal.id, payload); toast.success('Transport modifié !') }
      setModal(null); load()
    } catch (err) {
  ...
  const handleValider = async (id) => {
    try { await transportsApi.valider(id); toast.success('Validé !'); load() }
    catch { toast.error('Erreur') }
  }
  const handleRejeter = async (id) => {
    try { await transportsApi.rejeter(id); toast.success('Rejeté'); load() }
    catch { toast.error('Erreur') }
  }
  const handleDelete = async (id) => {
    if (!confirm('Supprimer ce service de transport ?')) return
    try { await transportsApi.delete(id); toast.success('Supprimé'); load() }
  ...
  const handleGalerie = async (e) => {
    ...
    fd.append('status', '1')
    try {
      await galeriesApi.createTransport(fd)
    ...
    <div className="admin-page">
      ...
      {loading ? <div className="center-spinner"><Spinner /></div> : (
        <table className="admin-table">
          <thead><tr><th>Nom</th><th>Adresse</th><th>Type</th><th>Région</th><th>Statut</th><th>Actions</th></tr></thead>
          <tbody>
              ...
              <tr key={transport.id}>
                ...
                <td>
                  <div className="admin-table__actions">
                    {transport.status !== 'valide' && <button className="admin-icon-btn admin-icon-btn--success" title="Valider" onClick={() => handleValider(transport.id)}><CheckCircle size={15} /></button>}
                    {transport.status !== 'rejete' && <button className="admin-icon-btn admin-icon-btn--danger" title="Rejeter" onClick={() => handleRejeter(transport.id)}><XCircle size={15} /></button>}
                  ...
                  {galModal.galeries.map(g => (
                    <div key={g.id} style={{ position: 'relative' }}>
                      ...
                      <button
                        ...
                        onClick={async () => { await galeriesApi.deleteTransport(g.id); load(); setGalModal(t => ({ ...t, galeries: t.galeries.filter(x => x.id !== g.id) })) }}
                      >✕</button>
                    </div>
                  ))}
                </div>
