...
import { responsablesApi, regionsApi, villesApi } from '../../api/services'
...
export default function ResponsableTransports() {
  const { user } = useAuth()
  ...
  const load = async () => {
    setLoading(true)
    try {
      const r = await responsablesApi.mesTransports()
  ...
  const handleDelete = async (id) => {
    ...
    try { await responsablesApi.deleteTransport(id); toast.success('Supprimé'); load() }
  ...
  const handleGalerie = async (e) => {
    ...
    try {
      await responsablesApi.createGalerieTransport(fd)
  ...
  const handleTrajet = async (e) => {
    e.preventDefault()
    try {
      await responsablesApi.createTrajet({
  ...
  const deleteTrajet = async (id) => {
    try {
      await responsablesApi.deleteTrajet(id)
                  ...
                  {galModal.galeries.map(g => (
                    <div key={g.id} style={{ position: 'relative' }}>
                      ...
                      <button
                        ...
                        onClick={async () => { await responsablesApi.deleteGalerieTransport(g.id); load(); setGalModal(t => ({ ...t, galeries: t.galeries.filter(x => x.id !== g.id) })) }}
                      >✕</button>
                    </div>
                  ))}
                </div>
