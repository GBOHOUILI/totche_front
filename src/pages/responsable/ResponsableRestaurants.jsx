...
export default function ResponsableRestaurants() {
  const { user } = useAuth()
  ...
  const load = async () => {
    setLoading(true)
    try {
      const r = await responsablesApi.mesRestaurants()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else { await responsablesApi.updateRestaurant(modal.id, payload); toast.success('Restaurant modifié !') }
      setModal(null); load()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try { await responsablesApi.deleteRestaurant(id); toast.success('Supprimé'); load() }
  ...
  const handleGalerie = async (e) => {
    ...
    try {
      await responsablesApi.createGalerieRestaurant(fd)
  ...
  const handlePlat = async (e) => {
    e.preventDefault()
    try {
      await responsablesApi.createPlat({
  ...
  const deletePlat = async (id) => {
    try {
      await responsablesApi.deletePlat(id)
          ...
          <div className="admin-modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="admin-form">
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--gray-500)' }}>
                {modal === 'create'
                  ? "Ce restaurant sera créé en attente - seul un admin peut le valider (pas vous, pas un autre responsable)."
                  ...
                  {galModal.galeries.map(g => (
                    <div key={g.id} style={{ position: 'relative' }}>
                      ...
                      <button
                        ...
                        onClick={async () => { await responsablesApi.deleteGalerieRestaurant(g.id); load(); setGalModal(r => ({ ...r, galeries: r.galeries.filter(x => x.id !== g.id) })) }}
                      >✕</button>
                    </div>
                  ))}
                </div>
