...
import { restaurantsApi, avisApi } from '../../api/services'
...
export default function RestaurantDetail() {
  ...
  useEffect(() => {
    avisApi.list({ id_restaurant: id, status: 'approuve' })
  ...
  const submitAvis = async (e) => {
    ...
    try {
      await avisApi.create({ id_restaurant: id, message: avisMessage.trim(), note: avisNote })
          ...
          <div className="detail-main">
            ...
            <section className="detail-section">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', marginBottom: '1rem' }}>
                ...
                <NoteResume moyenne={restaurant.note_moyenne} nombre={restaurant.nombre_avis} size={15} />
              </div>
              {avisList.length === 0 ? (
              ...
              {isAuthenticated && !avisJustSubmitted && (
                <form onSubmit={submitAvis} style={{ marginTop: '1.5rem' }}>
                  <label style={{ fontSize: '0.8rem', color: 'var(--gray-700)', display: 'block', marginBottom: '0.35rem' }}>
                    Vous avez mangé ici — laissez un avis
                  </label>
