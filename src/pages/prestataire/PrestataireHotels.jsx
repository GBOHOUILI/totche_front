...
export default function PrestataireHotels() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const r = await prestatairesApi.mesHotels()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modal === 'create') { await prestatairesApi.createHotel(payload); toast.success('Hôtel créé — en attente de validation') }
      else { await prestatairesApi.updateHotel(modal.id, payload); toast.success('Hôtel modifié !') }
      setModal(null); load()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try { await prestatairesApi.deleteHotel(id); toast.success('Supprimé'); load() }
  ...
  const handleGalerie = async (e) => {
    ...
    try {
      await prestatairesApi.createGalerieHotel(fd)
  ...
  const handleChambre = async (e) => {
    e.preventDefault()
    try {
      await prestatairesApi.createChambre({
  ...
  const deleteChambre = async (id) => {
    try {
      await prestatairesApi.deleteChambre(id)
          ...
          <div className="admin-modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="admin-form">
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--gray-500)' }}>
                {modal === 'create'
                  ? "Cet hôtel sera créé en attente — il deviendra visible publiquement une fois validé."
                  ...
                  {galModal.galeries.map(g => (
                    <div key={g.id} style={{ position: 'relative' }}>
                      ...
                      <button
                        ...
                        onClick={async () => { await prestatairesApi.deleteGalerieHotel(g.id); load(); setGalModal(h => ({ ...h, galeries: h.galeries.filter(x => x.id !== g.id) })) }}
                      >✕</button>
                    </div>
                  ))}
                </div>
